...
class SRGANDynamicModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # self.net_p = timm.create_model('mobilevitv2_050', pretrained=False)
        ...
        if self.is_train:
            ...
            self.load_key = self.opt['path'].get('param_key_g', None)
            if load_path_d is not None:
                self.load_network(self.net_d, load_path_d, self.opt['path'].get('strict_load_d', True), self.load_key)
        ...
        if self.ema_decay > 0:
            ...
            if load_path is not None:
                if 'pretrained_models' in load_path:
                    self.load_network_init_alldynamic(self.net_g_ema, load_path, self.opt['num_networks'], self.opt['path'].get('strict_load_g', True), self.load_key)
                else:
    ...
    def test(self):
        ...
        # paths = sorted(list(scandir(L_folder, full_path=True)))
        self.net_p.eval()
        with torch.no_grad():
            predicted_params, weights = self.net_p(self.lq)
        self.net_p.train()
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                ...
                # print("SR_GAN use ema ")
                # end.record()
                # torch.cuda.synchronize()
                # self.test_results['runtime'].append(start.elapsed_time(end))  # milliseconds
        else:
            self.net_g.eval()
            with torch.no_grad():
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        with torch.no_grad():
            ...
            for idx, val_data in enumerate(dataloader):
                ...
                img_name_ = osp.splitext(osp.basename(val_data['lq_path'][0]))[1]
            ...
            # ave_runtime = sum(self.test_results['runtime']) / len(self.test_results['runtime']) / 1000.0
            # print('------> Average runtime of  is : {:.6f} seconds'.format(ave_runtime))
        pbar.close()
